...
| `PosixAnsiTerminal.render()` | `ForegroundRuntime.run()` 每个前台周期 | 把运行快照刷新到 SSH ANSI 终端 | `_show()`、`_show_fault_code()` | 回到左上角 → 每行清除旧内容 → 写当前值 → 清除屏幕尾部 | 写 stdout 并刷新；不操作串口或 CAN | 写终端失败会由主循环按故障路径撤权并归零 | `test_operator_runtime.py::test_tui_render_clears_every_line_before_writing_shorter_values`；`test_tui_render_shows_fault_code_in_protocol_hex_and_decimal` |
...
| `LiftCalibrationSession.step()` | 起升标定命令源每周期 | 执行 40..80% 每级三次试验 | `_begin()`、`_observe()`、`_finish()`、`_fail()` | 新鲜度/方向/上限 → 300 ms动作 → 700 ms观察 → 记录有符号电流的最大幅值 | 只更新会话和返回命令；无直接 I/O | 失败锁存并永久返回全零 | `test_calibration.py::test_lift_session_runs_three_300ms_pulses_per_level_with_700ms_settle`；`test_lift_session_accepts_signed_current_and_records_peak_magnitude` |
