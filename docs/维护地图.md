...
| `pulse_on_s/pulse_wait_s` | 末端脉冲动作与首次/脉冲后全零观察节拍 | s | 动作复用 `LIFT_PULSE_S=0.100`；等待由 `response_delay_s+stable_time_s` 派生 | 动作固定 `0.100`、等待 `0.3..1.0` | `HeightController.__init__()`、`_terminal_pulse_command()` | 100 ms动作保持与现场标定同能量；响应延迟只改变全零观察，不再错误增加通泵量 | `test_controller.py::test_terminal_pulse_settles_before_first_on_and_waits_after_each_pulse`；`test_field_transition_from_continuous_lift_stops_before_terminal_pulse` |
