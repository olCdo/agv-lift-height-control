...
| `calibration.py` | 起升/下降试验会话、结果分析、最终标定包、上限测量 | CAN 线程和 SSH 输入 |
...
| `runtime_storage.py` | 版本化草稿、指纹和原子 JSON 保存；起升草稿 v2 只接受三次短脉冲结果 | 驱动硬件 |
...
| `current_multiplier/current_duration_s` | 同 PWM 二级过流保护 | 倍数、s | `1.5/0.2` | 倍数 `(1,1.5]`、持续时间 `(0,0.2]` | `HeightController._check_overcurrent()` | 用有符号泵电流的绝对值与标定峰值比较；改变已有标定后的持续过流门限，首次标定无此数据 | `test_controller.py::test_same_pwm_overcurrent_must_persist_for_200ms`；`test_negative_polarity_overcurrent_uses_current_magnitude` |
...
| `pulse_on_s/pulse_wait_s` | 末端脉冲动作与等待节拍 | s | `response_delay_s` 与 `stable_time_s` 派生 | 动作 `0.1..0.3`、等待 `0.3..1.0` | `HeightController.__init__()`、`_automatic_command()` | 标定响应延迟改变时调整末端逼近节拍 | `test_controller.py::test_terminal_pulse_uses_clamped_response_and_wait_phases` |
| `direction_tolerance_mm` | 累计反向容差 | mm | `1` | `(0,2]` | 控制器方向守卫、`LiftCalibrationSession.step()`、`LowerCalibrationSession.step()` | 改变反向锁故障灵敏度 | `test_calibration.py::test_lift_session_immediately_latches_reverse_motion_during_active_pulse` |
...
| 变量或字段 | 作用 | 单位 | 默认值或来源 | 有效范围 | 在哪个函数生效 | 修改后的影响 | 验证用例 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `LIFT_CALIBRATION_PWM` | 首版起升标定和成功结果的唯一 PWM | % | `40` | 固定 40 | `LiftCalibrationSession._current_pwm()`、`analyze_lift_trials()` | 改变三次标定动作以及新标定包的最小/粗升档；不能单独修改而不重做实机标定 | `test_calibration.py::test_lift_analysis_accepts_exactly_three_40_percent_trials`；`test_controller.py::test_single_level_calibration_never_commands_above_40` |
| `LIFT_CALIBRATION_REPEATS` | 固定短脉冲次数 | trial | `3` | 固定 3 | `LiftCalibrationSession.done`、`LIFT_CALIBRATION_PLAN` | 改变完成条件和草稿原始试验数量；旧 schema 不会自动迁移 | `test_calibration.py::test_lift_session_runs_three_100ms_pulses_with_700ms_settle`；`test_operator_runtime.py::test_calibration_draft_roundtrip_preserves_complete_trials` |
| `LIFT_PULSE_S` | 单次起升通电时间 | s | `0.100` | 固定 0.100 | `LiftCalibrationSession.step()`、`_finish()` | 改变单次动作位移、速度分母、启动延迟上限和机械风险 | `test_calibration.py::test_lift_session_runs_three_100ms_pulses_with_700ms_settle` |
| `LIFT_SETTLE_S` | 停泵后全零观察时间 | s | `0.700` | 固定 0.700 | `LIFT_TRIAL_S`、`LiftCalibrationSession.step()` | 改变正向最大上滑观测窗口；观察期授权失效不删除试验 | `test_calibration.py::test_lift_session_authorization_loss_during_settle_keeps_completed_pulse` |
| `DRAFT_SCHEMA_VERSION` | 起升动作草稿格式 | 版本号 | `2` | 当前只支持 2 | `CalibrationDraftStore.save_lift()/load_lift()` | v1 的 27 次草稿明确拒绝，防止把旧动作数据静默解释成三次短脉冲 | `test_operator_runtime.py::test_calibration_draft_rejects_old_v1_without_reinterpreting_it` |
...
| `PosixAnsiTerminal.render()` | `ForegroundRuntime.run()` 约每 200 ms | 把运行快照刷新到 SSH ANSI 终端 | `_show()`、`_show_fault_code()`、`_write_text()` | 构造完整画面 → 单次非阻塞完整帧写入 → 阻塞或部分写入直接丢帧 → 下一帧从左上角重绘 | 尽力写 stdout；不操作串口或 CAN；关闭时立即恢复 termios 和原 blocking 状态 | `BlockingIOError` 和部分帧只计数丢弃；其他写错误由主循环按故障路径撤权归零 | `test_operator_runtime.py::test_tui_render_clears_every_line_before_writing_shorter_values`；`test_posix_terminal_drops_blocked_frame_and_restores_terminal`；`test_posix_terminal_drops_partial_frame_and_next_render_is_complete` |
| `HeightController.step()` | 自动 `move` 或 `manual-lower` 命令源 | 普通闭环与人工下降仲裁 | `_begin_safety_cycle()`、`_automatic_command()`、`_record_actual_command()` | 输入门禁 → 电流幅值过流检查 → 故障处理 → 状态机 → 授权门控 | 更新状态、故障、方向和过流历史；无直接硬件 I/O | 返回全零并锁存故障 | `test_controller.py::test_each_input_guard_fails_closed`；`test_controller_accepts_signed_current_zero_offset`；`test_negative_polarity_overcurrent_uses_current_magnitude` |
...
| `LiftCalibrationSession.step()` | 起升标定命令源每 20 ms | 执行三次固定 40% 短脉冲 | `_begin()`、`_observe_powered()`、`_finish()`、`_fail()` | 等授权 → 100 ms 通电并检查方向/电流 → 700 ms 全零观察最高点 → 记录 → 再等授权 | 只更新会话和返回命令；无直接 I/O；观察期松开授权不删除完成脉冲 | 通电期撤权丢弃当前未完成试验；门禁失败锁存并永久返回全零；40% 不稳定时不会升级 PWM | `test_calibration.py::test_lift_session_runs_three_100ms_pulses_with_700ms_settle`；`test_lift_session_authorization_loss_during_settle_keeps_completed_pulse`；`test_lift_session_accepts_signed_current_and_records_peak_magnitude` |
...
flowchart LR
    ...
    M -->|标定或测量| X[Session或Survey]
    X --> Y[HeightController.step_external]
    C --> P[期望PumpCommand]
    Y --> P
...
首次标定没有最终 `CalibrationBundle`，因此 `controller=None`，但 `ForegroundRuntime` 仍统一执行控制循环、样本速度和 CAN 反馈门禁。已有标定后的重标定必须经过 `step_external()`，详见 `外部命令安全仲裁.md`。
...
- 不变量：自动目标只起升；授权缺失返回全零；外部模式下普通 `step()` 永远返回全零。
...
- 输入：样本、反馈和各自独立的死手授权。
- 职责：起升只生成三次 `40% × 100 ms` 试验并全零观察 700 ms；下降保持独立阀值序列；两者记录启动延迟、位移、速度、滑行和电流。
- 不变量：起升通电期撤权丢弃未完成试验，观察期撤权保留试验；输入无效、反向或越界时全零；40% 不稳定时不自动提高 PWM；失败会话不能继续放行。
- 主要测试：`test_calibration.py`。

### 草稿存储与确认

- `CalibrationDraftStore` 以 schema v2 保存三次短脉冲完整起升结果，并拒绝旧 v1 的 27 次草稿；
...
| `idle` | 最终标定和目标已准备 | 大误差且有 `u` 授权时进入 `coarse_lift` |
| `coarse_lift` | 误差大于 slow zone | 进入 slow zone 后转 `p_control` |
| `p_control` | 位于 slow zone | 进入 pulse zone 后转 `terminal_pulse` |
| `terminal_pulse` | 位于末端区 | 连续 500 ms 位于 ±2 mm 后转 `hold` |
| `hold` | 目标稳定 | 目标下方并重新授权时回到控制区 |
...
| `test_calibration.py` | 起升/下降序列、分析、方向、超时、越界和最终标定存储 |
...
| `test_application_modes.py`、`test_cli_application.py` | 各模式硬件隔离、草稿确认、前置失败和 CLI |
