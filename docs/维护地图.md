...
| `operator_runtime.py` | TTY、死手授权、传感器线程、信号桥、CSV 和单实例锁 | 选择业务模式 |
| `runtime_storage.py` | 版本化草稿、指纹和原子 JSON 保存 | 驱动硬件 |
...
| 变量或字段 | 作用 | 单位 | 默认值或来源 | 有效范围 | 在哪个函数生效 | 修改后的影响 | 验证用例 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `can.interface` | 选择 SocketCAN 接口 | 接口名 | `can0` | 1..15 字节且无空白 | `inspect_can_link()`、`_create_socketcan_bus()` | 改变只读检查和收发接口 | `test_can_pump.py::test_inspect_can_link_is_read_only_and_returns_parsed_state` |
...
| `current_multiplier/current_duration_s` | 同 PWM 二级过流保护 | 倍数、s | `1.5/0.2` | 倍数 `(1,1.5]`、持续时间 `(0,0.2]` | `HeightController._check_overcurrent()` | 改变已有标定后的持续过流门限；首次标定无此数据 | `test_controller.py::test_same_pwm_overcurrent_must_persist_for_200ms` |
...
| `height_mm` | 统一高度样本 | mm | `raw×轮周长/每圈计数` | `0..range_mm` | `read_sample()` 生成；控制器/标定器读取 | 影响目标、速度、方向和全部限位 | 同上 |
...
| `0x197 Byte3..7` | 泵反馈线格式 | 原始电流、故障码 | CAN 帧 | 8 字节标准帧 | `parse_pump_feedback()` | 改变反馈故障和电流保护输入 | `test_can_pump.py::test_parse_feedback_extracts_little_endian_fields_and_local_timestamp` |
...
| `ForegroundRuntime.run()` | `_run_mode()` 的 SSH 前台主线程 | 统一资源、按键、时序、门禁、TUI和日志 | `_handle_event()`、`_guard_motion_cycle()`、`_guard_height_sample()`、`_safe_cleanup()` | 启动 → 首步消费信号 → 采样/反馈 → 守卫 → 命令源 → 更新泵 → 记录 | 启停采样/CAN线程、写CSV、更新期望命令 | 任一异常先撤权和归零，再保留首个根因 | `test_foreground_loop.py::test_worker_tui_and_logger_errors_zero_before_stop` |
| `HeightController.step()` | 自动 `move` 或 `manual-lower` 命令源 | 普通闭环与人工下降仲裁 | `_begin_safety_cycle()`、`_automatic_command()`、`_record_actual_command()` | 输入门禁 → 故障处理 → 状态机 → 授权门控 | 更新状态、故障、方向和过流历史；无直接硬件 I/O | 返回全零并锁存故障 | `test_controller.py::test_each_input_guard_fails_closed` |
...
| `CanPump.start()` | `ForegroundRuntime.run()` | 只读链路检查、冲突预检并启动双线程 | `_run_preflight()`、`run_cycle()` | 检查链路 → 打开总线 → 被动监听 → 首次NMT/零 → 启动收发线程 | 打开 SocketCAN、发送 NMT/零、创建线程 | 事务式回滚、补零、关闭并拒绝不安全重启 | `test_can_pump.py::test_thread_start_failure_rolls_back_bus_and_started_thread` |
| `CanPump.run_cycle()` | 启动路径和 50 ms 发送线程 | 选择并发送最终实际命令 | `select_safe_command()`、`_send_payload()` | 读取原子状态 → 全部门禁 → 必要时NMT → 发送完整0x217 → 更新actual | 写 CAN；成功后更新 `last_sent_command` | 发送异常交给线程故障清理并保持全零策略 | `test_can_pump.py::test_run_cycle_keeps_zero_during_five_second_window_then_allows_fresh_command` |
...
| `LiftCalibrationSession.step()` | 起升标定命令源每周期 | 执行 40..80% 每级三次试验 | `_begin()`、`_observe()`、`_finish()`、`_fail()` | 新鲜度/方向/上限 → 300 ms动作 → 700 ms观察 → 记录 | 只更新会话和返回命令；无直接 I/O | 失败锁存并永久返回全零 | `test_calibration.py::test_lift_session_runs_three_300ms_pulses_per_level_with_700ms_settle` |
...
| `PassiveCanObserver.poll()` | `observe-can` 前台循环 | 有界接收0x197 | `parse_pump_feedback()` | 单轮最多64帧或2ms → 忽略噪声 → 返回最新反馈 | 只读 CAN，不发送 | 接收异常保存错误；持续噪声也有界返回 | `test_foreground_loop.py::test_passive_can_poll_returns_after_bounded_continuous_noise_then_reads_feedback` |
...
- 输入：命令源、可选运行时长和依赖对象。
- 职责：启动资源、消费信号/按键、统一 deadline/样本速度/CAN 反馈门禁、刷新命令、渲染并记录。
- 不变量：5 秒启动窗口不推进动作会话；事件日志不把“期望全零”伪报为“已发送全零”；清理路径先撤权和请求零。
...
- 不变量：`last_sent_command` 只在 `0x217` 成功发送后更新；任何门禁失败都发送完整全零。
- 主要测试：`test_can_pump.py`，其中纯策略、预检、线程生命周期和运行发送分别成组覆盖。

### `LiftCalibrationSession` 与 `LowerCalibrationSession`

- 输入：样本、反馈和各自独立的死手授权。
- 职责：生成固定试验序列并记录启动延迟、位移、速度、滑行和电流。
- 不变量：输入无效、反向、越界或撤权时全零；失败会话不能继续放行。
- 主要测试：`test_calibration.py`。

### 草稿存储与确认

- `CalibrationDraftStore` 保存完整起升结果；
...
| `test_can_pump.py` | 帧编解码、50 ms 调度、冲突、NMT、反馈、线程和归零 |
