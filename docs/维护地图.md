...
| `HeightController._automatic_command()` / `_terminal_pulse_command()` | `HeightController.step()` 的自动目标分支 | 切分控制区并生成末端相位命令 | `_terminal_pulse_on_s()`、`_reset_terminal_pulse()`、`_lift_command()` | 粗升/P控制 → 首次 `SETTLE` 全零 → 按误差选择50/100 ms并锁定 `ON` → `WAIT` 全零 → 完整观察后再决策；目标窗口返回液压保持 | 只生成不可变 `PumpCommand`；由 `CanPump` 异步写 `0x217` | 超调失败全零；超过5 mm或任一安全门禁锁故障全零 | `test_controller.py::test_field_transition_from_continuous_lift_stops_before_terminal_pulse`；`test_terminal_pulse_halves_action_inside_measured_coast_distance`；`test_terminal_settle_cannot_be_bypassed_by_small_downward_drift`；`test_overshoot_never_auto_lowers_and_faults_only_beyond_limit` |
