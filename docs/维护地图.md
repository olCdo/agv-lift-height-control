...
| `ForegroundRuntime.run()` | `_run_mode()` 的 SSH 前台主线程 | 统一资源、按键、时序、门禁、TUI和日志 | `_handle_event()`、`_guard_motion_cycle()`、`_guard_height_sample()`、`_safe_cleanup()` | 启动 → 首步消费信号 → 读取采样/反馈快照 → 取本周期单调时间 → 守卫 → 命令源 → 更新泵 → 记录 | 启停采样/CAN线程、写CSV、更新期望命令 | 任一异常先撤权和归零，再保留首个根因；异步快照不会被旧 `now` 误判为未来 | `test_foreground_loop.py::test_worker_tui_and_logger_errors_zero_before_stop`；`test_runtime_timestamps_cycle_after_reading_concurrent_can_snapshot` |
...
- 输入：命令源、可选运行时长和依赖对象。
- 职责：启动资源、消费信号/按键、先读异步快照再取本周期时间、统一 deadline/样本速度/CAN 反馈门禁、刷新命令、渲染并记录。
- 不变量：5 秒启动窗口不推进动作会话；异步快照不得被旧时间误判为未来；事件日志不把“期望全零”伪报为“已发送全零”；清理路径先撤权和请求零。
