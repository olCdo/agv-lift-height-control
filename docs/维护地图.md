...
| `emergency_stop.py` | 线程安全急停锁存、最终发送门禁、全零发送证据和传输故障状态 | 选择目标或直接访问 CAN |
| `lift_control.py` | 面向车辆上层的目标、更新、急停与解除公共门面 | 创建硬件、解析 Modbus 或另起发送线程 |
...
| `simulation.py` | 把命令按响应延迟平移后执行，并模拟PWM死区和停泵滑行的确定性仿真 | 生产硬件访问 |
...
| `overshoot_limit_mm` | 判定起升超调或下降下冲的锁故障门限 | mm | `5` | `(tolerance,5]` | `_lift_overshoot()`、`_lower_undershoot()` | 改变双向越界锁故障门限；容差外但未越过该值仍停止且不自动反向 | `test_controller.py::test_lift_overshoot_stops_without_auto_lower_and_faults_beyond_limit`；`test_lower_undershoot_stops_without_auto_lift_and_faults_beyond_limit` |
...
| `pulse_on_s/fine_pulse_on_s/pulse_wait_s` | 末端完整动作、近目标微动作与首次/脉冲后全零观察节拍 | s | 完整动作复用 `LIFT_PULSE_S=0.100`，微动作取一半 `0.050`；等待由 `response_delay_s+stable_time_s` 派生 | 动作 `0.050/0.100`、等待 `0.3..1.0` | `HeightController.__init__()`、`_terminal_pulse_on_s()`、`_terminal_pulse_command()` | 剩余误差不大于实测最大滑行量时减半单次能量；响应延迟只改变全零观察 | `test_controller.py::test_terminal_pulse_halves_action_inside_measured_coast_distance`；`test_terminal_pulse_settles_before_first_on_and_waits_after_each_pulse` |
| `_terminal_pulse_phase` | 区分首次停泵、通泵和脉冲后观察 | 状态 | 初始化和重置为 `None` | `None/SETTLE/ON/WAIT` | `_terminal_pulse_command()`、`_reset_terminal_pulse()` | 决定本周期为严格全零还是最低稳定PWM；通泵中撤权会重置，轻微回落也不能绕过完整观察 | `test_controller.py::test_terminal_authorization_loss_restarts_with_full_settle`；`test_terminal_settle_cannot_be_bypassed_by_small_downward_drift` |
| `_active_pulse_on_s` | 锁定当前ON相位选择的通泵时间 | s 或 `None` | 开始ON时由当前误差选择，退出/重置时清空 | `None/0.050/0.100` | `_terminal_pulse_command()`、`_reset_terminal_pulse()` | 防止一次微脉冲内部因高度变化动态切回完整脉冲；授权中断后不得沿用旧时长 | `test_controller.py::test_terminal_pulse_halves_action_inside_measured_coast_distance`；`test_terminal_authorization_loss_restarts_with_full_settle` |
...
| `lower_terminal_zone_mm` | 自动下降由连续开阀切入末端微脉冲的距离 | mm | `control` JSON；缺省与示例均为 `10.0` | `tolerance_mm + 0.001 .. 50.0` | `HeightController._automatic_lower_command()` | 增大后会更早停止连续下降并进入全零观察/短脉冲，通常更保守但到位更慢 | `test_config.py::test_load_legacy_control_config_uses_lowering_defaults`；`test_load_control_config_uses_explicit_lowering_values`；`test_controller.py::test_lower_terminal_zone_obeys_exact_settle_and_pulse_boundaries` |
| `lower_pulse_on_s` | 自动下降末端每次打开已确认舒适阀值的时长 | s | `control` JSON；缺省与示例均为 `0.05` | `0.05 .. 0.15` | `HeightController._lower_terminal_command()` | 增大将增加单次下降能量和下冲风险；到达边界的周期立即全零 | `test_config.py::test_load_legacy_control_config_uses_lowering_defaults`；`test_load_control_config_uses_explicit_lowering_values`；`test_controller.py::test_lower_terminal_zone_obeys_exact_settle_and_pulse_boundaries` |
| `lower_pulse_wait_s` | 进入末端区及每次下降脉冲后的完整全零观察时间 | s | `control` JSON；缺省与示例均为 `0.70` | `0.3 .. 1.0` | `HeightController._lower_terminal_command()` | 增大可观察更长液压惯性，但延长到位时间；减小会更早发下一脉冲 | `test_config.py::test_load_legacy_control_config_uses_lowering_defaults`；`test_load_control_config_uses_explicit_lowering_values`；`test_controller.py::test_lower_terminal_zone_obeys_exact_settle_and_pulse_boundaries` |

### 急停锁存运行字段

| 变量或字段 | 作用 | 单位 | 默认值或来源 | 有效范围 | 在哪个函数生效 | 修改后的影响 | 验证用例 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `EmergencyStopLatch._active` | 急停最高优先级门禁 | bool | 构造时 `False`；`trigger()` 置真，合格 `clear()` 置假 | `False/True` | `snapshot()`、`gate_command_for_send()`、`LiftHeightControl` 四个公共方法、`CanPump.update_command()/run_cycle()` | 为真时拒绝新目标、控制器进入 `EMERGENCY_STOP`、期望槽与最终发送均全零 | `test_emergency_stop.py::test_initial_snapshot_is_inactive_and_immutable`；`test_can_pump.py::test_emergency_stop_forces_next_cycle_zero_after_nonzero_command_was_sent`；`test_lift_control.py::test_emergency_stop_latches_first_reason_before_controller_and_blocks_targets` |
| `_reason` / `_triggered_at` | 保存本轮首次急停原因与触发单调时间 | 文本、s | 首次 `trigger(reason)` 和注入时钟；未触发为 `None` | 原因为非空白字符串；时间为有限非负数 | `trigger()`、`snapshot()`、`LiftHeightControl.emergency_stop()/update()` | 后续触发不得覆盖事故首因；坏时钟不会产生半锁存 | `test_emergency_stop.py::test_trigger_latches_first_reason_and_time`；`test_clock_exception_does_not_partially_trigger_and_retry_can_succeed`；`test_invalid_clock_result_does_not_partially_trigger_and_retry_can_succeed` |
| `_zero_sent_after_trigger` | 证明本轮急停后最后成功发送的是完整全零 | bool | 每轮 `trigger()` 重置为 `False`；`record_send_success()` 更新 | `False/True` | `record_send_success()`、`clear()`、`CanPump._send_command()` | 没有证据时禁止解除；急停后的非零成功帧会撤销证据 | `test_emergency_stop.py::test_clear_rejects_active_latch_without_post_trigger_zero_send`；`test_nonzero_send_after_zero_send_revokes_clear_evidence`；`test_can_pump.py::test_stop_zero_frame_is_post_trigger_send_evidence` |
| `_transport_fault` | 锁存急停期间的 CAN 发送故障 | 文本或空 | `record_transport_fault()`；显式 `record_transport_recovered()` 清空 | `None` 或非空白字符串 | `record_transport_fault()`、`record_transport_recovered()`、`clear()`、`CanPump._send_payload()` | 故障存在时即使已有全零证据也禁止解除，不能由解除动作顺带抹除 | `test_emergency_stop.py::test_transport_fault_must_be_explicitly_recovered_before_clear`；`test_can_pump.py::test_active_send_failure_latches_transport_fault_without_automatic_recovery` |
...
| `LIFT_PULSE_S` | 预充压、正式测量和闭环末端补高的单次通电时间 | s | `0.100` | 固定 0.100 | `LiftCalibrationSession.step()`、`_finish()`、`HeightController.__init__()`、`_terminal_pulse_command()` | 同时改变标定与闭环末端的单次机械能量、等效速度分母和硬件风险；不再单独作为运动响应搜索窗口 | `test_calibration.py::test_lift_session_precharges_then_records_three_delayed_measurements`；`test_controller.py::test_terminal_pulse_settles_before_first_on_and_waits_after_each_pulse` |
...
| `LIFT_MAX_RESPONSE_DELAY_S` | 正式样本可供闭环使用的最大首次运动延迟 | s | `0.300` | `(0,0.300]` | `LiftCalibrationSession._finish()`、`analyze_lift_trials()` | 收紧会拒绝慢响应机构；放宽会延长末端全零观察并降低响应确定性，但不会增加通泵时间 | `test_calibration.py::test_lift_analysis_rejects_response_later_than_controller_limit` |
...
| `HeightController._automatic_command()` | `HeightController.step()` 的自动目标分支 | 按目标误差选择保持、起升或下降，阻止同一轮越界后自动反向 | `_hold_command()`、`_automatic_lift_command()`、`_automatic_lower_command()`、`_lift_overshoot()`、`_lower_undershoot()` | 容差内保持；目标在上方走起升；目标在下方走下降；接近方向已确定后若越过目标则全零/故障 | 更新接近方向、双向末端相位和试验失败状态；只生成命令，无直接 I/O | 起升超调或下降下冲立即全零，超过 `overshoot_limit_mm` 锁故障 | `test_controller.py::test_target_below_current_height_commands_confirmed_lower_valve`；`test_automatic_command_never_combines_lift_and_lower_outputs`；`test_lift_overshoot_stops_without_auto_lower_and_faults_beyond_limit`；`test_lower_undershoot_stops_without_auto_lift_and_faults_beyond_limit` |
| `LiftHeightControl.set_target_height()` | CLI `move` 装配或未来车辆/Modbus 命令适配器 | 设置新的自主双向高度目标 | `EmergencyStopLatch.snapshot()`、`HeightController.set_target()` | 先检查急停 → 校验目标/临时上限 → 重置控制接近状态 | 写控制器目标和有效上限；无直接硬件 I/O | 急停中拒绝；目标或上限非法时保持失败关闭 | `test_lift_control.py::test_update_runs_bidirectional_automatic_control_without_keyboard_authorization`；`test_emergency_stop_latches_first_reason_before_controller_and_blocks_targets` |
| `LiftHeightControl.emergency_stop()` | 未来车辆安全回路/上层主动调用；当前 CLI 无 `e` 键 | 先锁最终发送门禁，再撤销控制器全部运动意图 | `EmergencyStopLatch.trigger()/snapshot()`、`HeightController.enter_emergency_stop()` | 先触发共享锁存 → 取得门面锁 → 复核本轮锁存 → 控制器进入急停 | 锁存首因；清除目标、人工下降、外部模式和脉冲状态 | 非空白原因校验失败则不触发；重复触发不覆盖首因 | `test_lift_control.py::test_emergency_stop_latches_first_reason_before_controller_and_blocks_targets`；`test_emergency_stop.py::test_trigger_latches_first_reason_and_time` |
| `LiftHeightControl.clear_emergency_stop()` | 未来车辆恢复流程主动调用；无键盘解除入口 | 仅在健康观测、全零证据和传输恢复后原子解除 | `_validated_now()`、`_validate_age()`、`EmergencyStopLatch.state_transition_guard()/clear()`、`HeightController.exit_emergency_stop()` | 同步泵侧急停 → 检查缓存样本/反馈/普通故障 → 原子解除锁存与控制状态 | 清空急停字段，控制器回 `MONITOR` 或恢复此前普通 `FAULT`；目标保持为空 | 任一证据、观测或状态变化不合格都保持 `EMERGENCY_STOP` | `test_lift_control.py::test_healthy_clear_does_not_restore_the_old_target_or_output`；`test_clear_rejects_unhealthy_cached_inputs_without_partially_releasing`；`test_clear_requires_zero_send_evidence_and_a_recovered_transport`；`test_trigger_waits_until_clear_and_controller_exit_finish_as_one_transition` |
| `LiftHeightControl.update()` | `MoveCommandSource.step()` 每个前台 20 ms 周期；未来车辆循环 | 缓存观测并执行无需键盘授权的自主双向控制 | `EmergencyStopLatch.snapshot()`、`HeightController.enter_emergency_stop()/step()` | 缓存样本/反馈 → 同步泵侧急停 → 固定双向自动授权调用控制器 | 更新解除急停所需观测缓存和控制器状态；返回期望命令，无直接 I/O | 缺失观测由调用方先全零；急停或控制门禁失败返回全零 | `test_lift_control.py::test_update_runs_bidirectional_automatic_control_without_keyboard_authorization`；`test_update_imports_a_pump_side_emergency_stop_into_the_controller`；`test_foreground_loop.py::test_move_and_manual_lower_sources_use_controller_step_chain` |
| `HydraulicLiftSimulator.advance()` | 仿真测试循环 | 无等待地推进一段液压时间 | `_command_target_velocity()`、`_integrate_delayed_history()`、`_integrate_coast()` | 保存当前命令速度段 → 将延迟前的历史区间平移到当前动作 → 有命令按目标速度积分，无命令按指数滑行 | 只更新内存中的时间、高度、速度和反馈；全零不会删除尚未到达机械端的旧脉冲 | 非法同时升降或时间步无效时抛错，不访问真实硬件 | `test_simulation.py::test_simulator_preserves_delayed_lift_after_command_returns_zero`；`test_controller_reaches_three_targets_in_deterministic_hydraulic_simulation` |
...
| `CanPump.run_cycle()` 最终门禁 | 启动路径和唯一 50 ms 发送线程 | 把普通安全策略和急停原子门禁合并为最终实际命令 | `select_safe_command()`、`_send_command()`、`EmergencyStopLatch.gate_command_for_send()` | 原子快照 → 急停优先或普通 NMT/新鲜度/反馈门禁 → 必要时NMT → 发送门前再次原子检查急停 → 同步发送完整 `0x217` → 记录证据/实际命令 | 唯一实际 CAN 写入口；成功后更新 `last_sent_command`，急停时清空期望槽 | 发送异常记录传输故障并由线程故障清理；任何门禁失败全零；触发与发送竞争时触发返回后不再放行非零 | `test_can_pump.py::test_trigger_between_early_selection_and_send_gate_forces_same_cycle_zero`；`test_trigger_waits_for_nonzero_send_gate_then_next_cycle_is_zero`；`test_emergency_stop_reason_has_priority_over_all_ordinary_can_gates` |
| `_run_mode()` / `_build_control_source()` / `default_dependencies()` 应用装配 | `run_application()` | 为一次前台会话创建共享急停锁、公共门面、命令源和同一个 `CanPump` | `EmergencyStopLatch()`、`LiftHeightControl.set_target_height()`、`MoveCommandSource()`、`pump_factory()` | 创建一把共享锁存 → `move` 构造门面并写目标 → 泵工厂接收同一锁存 → 运行时把命令写入唯一泵槽 | 打开所需传感器/CAN，创建一个 CAN 发送线程；`move` 不开放 u/d 授权 | 标定/目标前置校验失败时不构造硬件；共享实例断言失败由测试捕获 | `test_application_modes.py::test_move_builds_autonomous_source_and_shares_latch_with_pump` |
...
```mermaid
flowchart LR
    A["CLI 或未来车辆命令"] --> B["LiftHeightControl.set_target_height"]
    B --> C["HeightController.set_target 校验目标和上限"]
    D["20 ms 样本与 0x197 反馈"] --> E["LiftHeightControl.update 固定双向自动授权"]
    C --> E
    E --> F{"目标误差"}
    F -->|目标更高| G["自动起升"]
    F -->|目标更低| H["连续下降或末端全零与短脉冲"]
    F -->|容差内| I["液压保持"]
    G --> J["CanPump 唯一命令槽"]
    H --> J
    I --> J
...
```mermaid
sequenceDiagram
    participant U as "车辆上层或安全输入"
    participant L as "EmergencyStopLatch"
    participant C as "HeightController"
    participant P as "CanPump 发送线程"
    U->>L: "emergency_stop 先 trigger"
    L-->>P: "最终发送门禁立即只允许全零"
    U->>C: "enter_emergency_stop 撤销全部运动意图"
    P->>L: "成功发送完整全零后记录证据"
    Note over L,C: "状态保持 EMERGENCY_STOP；新目标被拒绝"
```

```mermaid
flowchart LR
    A["集成层请求 clear_emergency_stop"] --> B{"本轮全零发送成功且传输恢复"}
    B -->|否| X["保持 EMERGENCY_STOP"]
    B -->|是| C{"样本和 0x197 新鲜健康且无普通故障"}
    C -->|否| X
    C -->|是| D["原子 clear 锁存并退出控制器急停"]
    D --> E["MONITOR 或恢复原普通 FAULT"]
    E --> F["旧目标不恢复；必须下发新目标"]
```

三图只描述本次高风险路径。标定、Survey、退出和首次构造的详细触发者、门禁与失败行为保留在变量表和函数表中。
...
- 不变量：`move` 可按目标误差自动选择升降，但同一轮越过目标后禁止自动反向；人工/外部动作没有对应死手授权时必须全零；目标稳定窗口可保持 Byte0 为1；外部模式下普通 `step()` 永远返回全零。
- 主要测试：`test_controller.py`，其中普通控制、安全门禁和外部模式分别成组覆盖。

### `LiftHeightControl` 公共门面

- 入口：CLI `move`，以及未来车辆目标、急停、解除适配器。
- 职责：用同一把门面锁串行化 `set_target_height()`、`update()`、`emergency_stop()`、`clear_emergency_stop()`。
- 内部调用：`HeightController` 的目标/周期/急停方法和共享 `EmergencyStopLatch`。
- 状态与硬件副作用：自身不写 CAN；目标写入控制器，急停先锁最终发送门禁并撤销全部意图，更新缓存解除所需观测。
- 失败行为：急停中拒绝目标；解除缺少全零证据、传输恢复、新鲜样本、健康反馈或存在普通故障时保持锁存。
- 主要测试：`test_lift_control.py`；应用共享锁装配见 `test_application_modes.py::test_move_builds_autonomous_source_and_shares_latch_with_pump`。

### `CanPump.run_cycle()`

- 输入：最新期望命令；后台发送默认在原子状态快照后自行读取本机单调时间，显式时间仅供启动首周期和确定性测试。
- 职责：按急停、启动窗口、命令新鲜度、反馈新鲜度、反馈故障和线程故障选择实际命令，再在同步发送前通过 `gate_command_for_send()` 二次原子检查急停，发送 NMT 和完整 `0x217`。
- 不变量：`last_sent_command` 只在 `0x217` 成功发送后更新；触发急停返回后不再放行旧非零帧；急停期成功全零才形成解除证据；任何门禁失败都发送完整全零。
...
| `idle` | 最终标定和目标已准备，或正在目标窗口内计时 | `move` 按误差自主进入起升或下降控制区；目标窗口内发送互锁保持并在500 ms后进入 `hold` |
| `coarse_lift` | 误差大于 slow zone | 进入 slow zone 后转 `p_control` |
| `p_control` | 位于 slow zone | 进入 pulse zone 后转 `terminal_pulse` |
| `terminal_pulse` | 位于末端区 | 首次 `SETTLE` 全零 → 按剩余误差选择50/100 ms `ON` → `WAIT` 全零；进入 ±2 mm 后转 `idle` 稳定计时 |
| `coarse_lower` | 当前高度高于目标超过 `lower_terminal_zone_mm` | 连续输出已确认舒适下降阀；进入末端区转 `lower_settle` |
| `lower_settle` | 自动下降进入末端区或脉冲结束 | 按 `lower_pulse_wait_s` 全零，随后转 `lower_pulse` |
| `lower_pulse` | 下降末端短脉冲 | 按 `lower_pulse_on_s` 输出舒适阀，边界周期立即全零并回 `lower_settle` |
| `hold` | 连续 500 ms 位于目标 ±2 mm | 持续发送 `217#0100000000000000`；`move` 漂移超过容差后无需 `u`/`d`，自动按误差方向重新进入起升或下降控制区。标定、Survey、预升和人工下降仍遵守各自死手授权边界 |
| `manual_lower` | 人工下降模式且有 `d` 授权 | 撤权后输出全零并等待 |
| `fault` | 任一安全门禁失败 | 条件恢复且明确请求清除后回到空闲状态 |
| `emergency_stop` | `LiftHeightControl.emergency_stop()` 或共享泵侧锁存同步进入 | 持续严格全零；合格解除后进入 `monitor`，若急停前已有普通故障则恢复 `fault`；旧目标不恢复 |
...
| `test_simulation.py` | 命令平移式液压延迟、停泵后延迟动作、最小PWM死区、滑行、误差和超调 |
...
先在 `simulation.py` 和控制器单元测试中证明误差、超调、授权与故障边界，再改 `HeightController`。自动 `move` 只允许使用最终人工确认的舒适下降阀值；禁止使用未确认阀值、禁止升降同时输出，也禁止引入第二个 `0x217` 发送者。

## 不可破坏的安全不变量

- `move` 自主双向控制不依赖键盘授权；标定、Survey、预升和人工下降没有对应死手授权时，实际运动输出必须为零。
- 自动定高可以按目标误差选择起升或下降，但同一轮起升超调不得自动下降修正，同一轮下降下冲不得自动起升修正。
- 同一周期只能有一个命令源，起升和下降不能同时非零。
- 未收到健康、足够新的 `0x197`，不能发送非零动作。
- 5 秒 NMT 窗口不能推进标定、Survey 或自动状态机。
- TUI/CSV 的实际命令只能来自最后一次成功发送的 `0x217`。
- 急停必须同时门控控制器和最终发送；解除必须有触发后的全零发送证据、传输恢复及新鲜健康观测，且不得恢复旧目标。
- 当前独立 CLI 无 `e` 键，车辆上层急停触发链尚未实机验证。
