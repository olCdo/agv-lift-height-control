...
这是一个面向 Orange Pi 5 Plus 与 Ubuntu 的前台测试程序，用 Modbus RTU 拉绳位移传感器读取升降高度，并通过 `can0` 控制液压泵。当前用于现场标定、自主双向闭环定高、人工下降和安全上限测量。

`move` 只接收目标高度，并根据当前高度自动选择起升或下降；本轮起升超调后不会自动反向下降，本轮下降下冲后也不会自动反向起升。标定、`survey-upper`、`prepare-lower` 和 `manual-lower` 仍使用 `u` 或 `d` 死手授权。输入结束、信号、线程异常、安全门禁失败或安全退出都会请求发送完整全零命令。
...
- 自动下降：目标低于当前高度时，先连续打开已确认的舒适下降阀，进入末端区后按“全零观察—短脉冲”接近目标。
- 越界：本轮起升超调或下降下冲都会立即归零，超过 5 mm 锁存故障，并禁止在同一轮自动反向修正。
- 上限：程序绝对保护不允许超过 2900 mm；没有持久软限位时，起升相关命令必须人工输入临时上限。
- SSH：只允许真实前台终端，拒绝 `tmux`、`screen`、`nohup`、后台进程和 `TERM=dumb`。
- 急停：公共门面可进入 `EMERGENCY_STOP`，锁存期间控制层和最终 CAN 发送门禁都只允许完整全零。
...
`move` 模式不需要持续按 `u` 或 `d`，这两个键在该模式不形成运动授权；程序根据目标与当前高度自主选择方向，`q` 仍会请求全零并安全退出。其他动作维护模式的 `u`/`d` 采用续期而不是切换，必须按对应章节持续或逐次操作，停止重复后授权自动过期。当前没有 `e` 急停键；急停只通过下文公共函数触发。不要通过 `tmux`、`screen`、`nohup`、`systemd` 或 shell 后台运行。
...
| `move` | 是 | 是 | 只给目标高度，自动选择起升或下降；没有持久软限位时还要给临时上限 |
...
`move` 启动后不需要持续按 `u` 或 `d`；只选择已确认安全的低高度目标并至少重复三次，
需要停止时按 `q` 安全退出。目标高于当前高度时，粗升、P 区和末端脉冲使用已标定 PWM。
从连续起升进入末端区时，控制器先发送全零并完成一次
停泵观察，再从静止状态发出短脉冲：剩余误差大于实测最大滑行量时使用 100 ms，
进入最大滑行量范围后使用 50 ms 微脉冲。实测响应延迟只延长全零观察时间，不会延长
通泵时间。进入目标 ±2 mm 稳定窗口后，正常保持帧为
`217#0100000000000000`；连续 500 ms 位于窗口内后进入 `hold`。故障、超时、退出和命令
过期仍发送 `217#0000000000000000`。TUI 的“互锁=开/关”对应 Byte0。超过目标 2 mm
会判本轮起升失败并保持全零，超过 5 mm 会锁存故障，不能用自动下降掩盖本轮起升失败。

目标低于当前高度时，控制器使用最终标定包中已确认的舒适下降阀值；距离大于
`lower_terminal_zone_mm` 时连续下降，进入末端区后先按 `lower_pulse_wait_s` 保持全零，
再按 `lower_pulse_on_s` 发出短脉冲。下降越过目标超过容差后同样停止，不能在本轮自动
反向起升。`manual-lower` 仍是独立的 `d` 键死手维护模式。
...
车辆上层或安全输入适配器应调用 `LiftHeightControl.emergency_stop(reason)`。该调用先锁存
底层发送门禁，再撤销目标、人工下降和外部模式，使控制器进入 `EMERGENCY_STOP`；此后
设置目标会被拒绝，最终发送层只允许完整全零。当前 CLI 没有 `e` 键，现有独立程序也没有
接入车辆安全回路，因此“车辆上层触发 → 软件急停 → 实际 `0x217` 全零”的链路尚未实机验证。

解除必须由集成层调用 `LiftHeightControl.clear_emergency_stop()`，且需同时具备本轮急停后的
全零发送成功证据、传输已恢复、新鲜有效高度样本、新鲜无故障 `0x197` 反馈，并且没有被
急停覆盖的普通控制器故障。解除急停后必须重新下发目标；旧目标、人工下降和外部命令都
不会自动恢复。
...
未来并入 `kinco_duolun` 时，车辆进程必须成为 `0x217` 的唯一发送者：把现有固定全零
keepalive 替换为本项目仲裁后的完整命令槽，不能再新增第二个 `0x217` 发送线程。目标、
急停和解除统一经过 `LiftHeightControl` 公共门面；Modbus TCP 命令还必须使用命令序列号
或“新写”握手，保证解除急停不会让急停前的旧目标自动复活。详见
[外部命令安全仲裁](docs/外部命令安全仲裁.md)。
...
测试覆盖寄存器组合、pymodbus 3.x API 兼容、CAN 编解码与调度、控制状态机、停泵后仍执行旧脉冲的液压延迟仿真、SSH 授权、信号/EOF 停机、外部 `0x217` 冲突、版本化草稿和无硬件确认路径。GitHub Actions 在 Python 3.10 与 3.12 上运行同一组测试。
