...
| `calibrate-lift` | 是 | 是 | 3 次固定 40% 短脉冲标定，强制输入临时最大高度，完成后保存起升草稿 |
...
首版只执行 3 次固定 40% PWM，不会自动升到 45%～80%。每次通电 100 ms，随后保持
CAN 全零观察 700 ms；三次全部完成后才结束标定。临时上限、已有持久软限位、配置
绝对上限和 2900 mm 中的最小值生效。

每次准备动作时敲击一次 `u` 后松开；单个字符的 700 ms 授权足以覆盖 100 ms 通电，
观察期可以松开且不会丢弃已经完成的脉冲。不要在 700 ms 观察期持续重复 `u`，等待本次
约 800 ms 的完整试验结束后，再为下一次动作重新敲击 `u`。若 100 ms 通电阶段授权提前
失效，本次不完整记录会丢弃并保持全零。

每次要求 100 ms 通电位移至少 1 mm 且方向一致。三次任一不满足时本次标定失败，不会
自动提高 PWM。CSV 即使中断或失败也会保留；只有三次全部成功才写入
`lift-calibration-draft.json`。旧版 schema v1 起升草稿不会被静默复用，更新代码后必须按
本节重新标定以生成 schema v2 草稿。
